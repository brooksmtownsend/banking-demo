...
use exports::demo::image_analyzer::analyzer::Guest;
...
impl Ollama {
    fn do_detect(image: Vec<u8>) -> Result<bool, String> {
        log(Level::Info, "Detect", "Initializing");
        ...
        log(Level::Info, "Detect", "Preparing request");
        ...
        log(Level::Info, "Detect", "Sending request");
        let resp = req.fetch_bytes(contents).map_err(|e| e.to_string())?;

        log(Level::Info, "Detect", "Parsing response");
...
impl OutgoingRequest {
    fn fetch_bytes(self, payload: Vec<u8>) -> Result<Bytes> {
        ...
        log(Level::Info, "fetch_bytes", "Writing body to stream");
        ...
        log(Level::Info, "fetch_bytes", "Finishing body to stream");
        drop(body_stream);
        OutgoingBody::finish(body_writer, None).expect("failed to finish body");

        log(Level::Info, "fetch_bytes", "blocking stream");
        resp.subscribe().block();

        log(Level::Info, "fetch_bytes", "getting response");
        let response = resp
            .get()
        ...
        log(Level::Info, "fetch_bytes", "consuming response");
