...
func Detect(ctx__ context.Context, wrpc__ wrpc.Invoker, image []uint8) (r0__ *wrpc.Result[bool, string], err__ error) {
	...
	if err__ != nil {
		err__ = fmt.Errorf("failed to write `image` parameter: %w", err__)
		return
	}
	if write0__ != nil {
		err__ = errors.New("unexpected deferred write for synchronous `image` parameter")
		return
	}
	var w__ wrpc.IndexWriteCloser
	var r__ wrpc.IndexReadCloser
	w__, r__, err__ = wrpc__.Invoke(ctx__, "demo:image-analyzer/analyzer@0.0.1", "detect", buf__.Bytes())
	if err__ != nil {
	...
	defer func() {
		if err := r__.Close(); err != nil {
			slog.ErrorContext(ctx__, "failed to close reader", "instance", "demo:image-analyzer/analyzer@0.0.1", "name", "detect", "err", err)
		}
	}()
	if cErr__ := w__.Close(); cErr__ != nil {
		slog.DebugContext(ctx__, "failed to close outgoing stream", "instance", "demo:image-analyzer/analyzer@0.0.1", "name", "detect", "err", cErr__)
	}
	r0__, err__ = func(r wrpc.IndexReadCloser, path ...uint32) (*wrpc.Result[bool, string], error) {
